<?php
class ModelExtensionPaymentEasebuzz extends Model
{
    public function getMethod($address, $total)
    {
        ...
        if ($this->config->get('payment_easebuzz_total') > 0 && $this->config->get('payment_easebuzz_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_easebuzz_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                ...
                'terms' => '',
                'sort_order' => $this->config->get('payment_easebuzz_sort_order')
            );
        }
        return $method_data;
    }
